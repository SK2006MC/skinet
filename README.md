...
Relativity Network is an experimental neural architecture that replaces global optimisation (gradient descent / backpropagation) with local state reconciliation. It treats learning not as a search for a minimum in a loss landscape, but as a physical system seeking homeostatic equilibrium.
...
a = \text{ReLU}\bigl(b\,(b - x)\bigr)

Condition State Physical Meaning
...
Instead of propagating an error gradient, the network performs State Reconstruction. It treats a desired output as a boundary condition and reconstructs a compatible internal state layer‑by‑layer.
...
W_{ij} \leftarrow \sqrt{r_i\, r_j}\; \cdot\; W_{ij}

V_{ji} \leftarrow \frac{1}{\sqrt{r_i\, r_j}}\; \cdot\; V_{ji}
...
b \leftarrow b + \alpha\,(x^* - b), \qquad \alpha \ll 1
...
· Readout Mechanism – How to extract decisions from a network that converges to silence? Possible answer: silence on a familiar sample is the classification, or a separate decoding layer driven by the pre‑convergence state.
...
The Relativity Network is not an optimizer; it is an equilibrator. It posits that intelligence is not the ability to find a correct answer, but the ability to build an internal model so accurate that the external world ceases to be a surprise.
