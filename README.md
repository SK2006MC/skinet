...
Relativity Network is an experimental neural architecture that replaces global optimisation (gradient descent / backpropagation) with local state reconciliation. It treats learning not as a search through weight space, but as a process of iterative equilibration—bringing the internal model into harmony with the external world.
...
Inspired by depletion‑mode transistors, the Relativity neuron is normally ON. It possesses an intrinsic tendency to fire based on its memory `b`. Incoming evidence `x` does not create activity — instead, it modulates surprise. The neuron fires strongest when the world contradicts its expectations.
...
Instead of propagating an error gradient, the network performs **State Reconstruction**. It treats a desired output as a boundary condition and reconstructs a compatible internal state layer-by-layer, bottom-to-top.

To avoid the O(n³) cost of a pseudo‑inverse, the network maintains a **Mirrored Reciprocal Matrix** `V` that evolves alongside the weight matrix `W` and approximates `W⁺`.
...
| Mechanism | Reciprocal `W ↔ V` | Dendritic feedback loops |
...
- **Readout Mechanism** – How to extract decisions from a network that converges to silence? Possible answer: silence on a familiar sample is the classification, or a separate decoding layer driven by activity patterns.
...
- **Sparsity** – Implementing hard‑pruning for connections where `r ≈ 1` over extended periods, reflecting "fully compressed" knowledge.
...
The Relativity Network is not an optimizer; it is an equilibrator. It posits that intelligence is not the ability to find a correct answer, but the ability to build an internal model so accurate that it becomes indistinguishable from reality. When the model is perfect, the system goes silent.
